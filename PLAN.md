...
| 2 | Prompt list | C2 | |
| 3 | Queue a run | C3 | |
| 4 | Worker: claim, concurrency, retry | C4, C6 | |
...
- **GitHub repository created** - blocks the first deploy in Phase 0.
...
Includes the gate command, `.env.example`, and the first deploy.
...
`SELECT ... FOR UPDATE SKIP LOCKED`, a minimal `mentions.ts` and `citations.ts`,
one run page, and `scripts/verify-live.ts`.
...
- Done when: WHEN a prompt list is saved, every non-empty line becomes one ordered
  prompt for that company; IF the list exceeds 50 prompts, a warning naming the
  resulting call count is returned and the save still succeeds - verified by
  `npm run test -- prompts` and `npm run verify`.
...
- Done when: WHEN a run is started, a run record with status `queued` exists
  carrying an immutable copy of the prompt texts, an immutable target list, the
  chosen N and the prompt-set hash, and the request returns without waiting for
  the measurement; editing the company's prompts afterwards leaves that run
  unchanged - verified by `npm run test -- runs.queue` and `npm run verify`.
- Commit: `feat: queue a run`

## Phase 4 - Worker: claim, concurrency, retry

- Delivers: C4, C6
- Done when: WHILE a run is `queued`, exactly one worker claims it and sets it to
  `running`, and every (prompt, target, repetition) combination is attempted at no
  more than `RUN_CONCURRENCY` calls in flight; IF a call fails with a rate limit, a
  timeout or a 5xx, it is retried up to three times with exponential backoff before
  being recorded as failed - verified by `npm run test -- worker.claim` (two
  concurrent claimers, exactly one winner) and `npm run test -- retry`.
- Commit: `feat: worker claim and retry`
...
- Delivers: C8
- Done when: WHEN an answer is recorded successfully, the recognised brands in it
  are persisted with the subject brand's 1-based position and the total number of
  recognised brands found - verified by `npm run test -- mentions` against
  fixtures covering alias variants, casing, punctuation, absence, and an empty
  competitor list.
...
- Done when: mention rate, average position and competitor frequency are computed
  at read time with no aggregate stored as a column; every displayed figure carries
  its run's coverage; IF coverage is below `COVERAGE_THRESHOLD` the run is labelled
  unreliable rather than presented as a measurement and remains visible; IF every
  attempt for one prompt failed that prompt reads "no data" and never "not
  mentioned"; and WHEN a run reaches a terminal status its total token usage and
  total cost are displayed - verified by `npm run test -- aggregate` with fixtures
  at 100, 85, 79 and 0 percent coverage, and by inspecting the run page.
- Commit: `feat: aggregation, coverage and cost`

## Phase 8 - Comparability guard

- Delivers: C11
- Done when: IF two runs of one company differ in prompt-set hash or in target
  list, THEN they are not presented as one series and the change of measurement
  basis is stated - verified by `npm run test -- comparability` with three runs:
  identical basis, changed prompt, changed model id.
...
- Done when:
  - A run of the operator-supplied real brand with at least ten real prompts has
    completed at coverage of 80 percent or higher, with at least one citation on
    every successful answer.
  - Every figure on screens 1 to 3 is readable without ambiguity: no percentage
    appears without its coverage, no failed prompt reads as a zero, and a run whose
    basis changed says so.
  - **Stability check:** the same run is repeated at least a day later on the same
    basis. IF the two results differ by more than one step of N, THEN N is raised
    and `ARCHITECTURE.md` and `SPEC.md` are updated before Phase 10 begins. This
    resolves the open question in `SPEC.md` about whether N=3 is believable.
...
- Done when: `npm run verify` exits 0 against the deployed configuration; all
  fourteen capabilities pass their EARS criteria; the real-brand run from Phase 9
...
- **Historical trend charts.** Runs are already immutable and hashed on prompt set
  and targets, so the series exists; only the chart is missing.
